...
type connectionInfo struct {
	...
	BastionUser       string `mapstructure:"bastionUser"`
	BastionPassword   string `mapstructure:"bastionPassword"`
	BastionPrivateKey string `mapstructure:"bastionPrivateKey"`
	BastionHost       string `mapstructure:"bastionHost"`
	BastionPort       int    `mapstructure:"bastionPort"`

	// Deprecated
	KeyFile        string `mapstructure:"keyFile"`
	BastionKeyFile string `mapstructure:"bastionKeyFile"`
}
...
func parseConnectionInfo(ci types.ConnInfo) (*connectionInfo, error) {
	...
	if err := dec.Decode(ci); err != nil {
		log.Warningf("ci: %v", ci)
		log.Warningf("error parseConnectionInfo: %v", err)
		return nil, err
	}
	...
	// shouldn't try.
	if ci["agent"] == "" && os.Getenv("SSH_AUTH_SOCK") != "" {
		connInfo.Agent = true
	}
...
func safeDuration(dur string, defaultDur time.Duration) time.Duration {
	...
	if err != nil {
		log.Warningf("Invalid duration '%s', using default of %s", dur, defaultDur)
		return defaultDur
	}
	return d
}
